...
extern struct table2D idleVCorrectionTable; //6 bin idle voltage correction (2D)
...
struct statuses {
  ...
  byte IacbatCorrection; /**< The amount of iac voltage enrichment currently being applied */
  byte BIEGActiveCorrection;
  ...
  byte knockRetard;
  bool BIEGActive;   /**< Whether the externally controlled closed throttle position sensor is currently active */
...
struct config15 {
  ...
  byte idleBat[6]; //Correction table for idle PW vs battery voltage
  byte voltage2CorrectionBins[6]; //X axis bins for voltage correction tables
  byte zaworRPM;      /// RPM must be above this for closed loop to function
  byte zaworTPSMax;   /// TPS must be below this for closed loop to function
  byte BIEGPin : 6;
  byte BIEGPolarity : 1;
  byte BIEGEnabled : 1;
...
extern byte pinBIEG; //Input for triggering Idle Up
...
extern byte pinGEAR; //Input for triggering closed throttle state
