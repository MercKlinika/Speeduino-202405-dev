...
uint16_t correctionsFuel()
{
  ...
  currentStatus.IacbatCorrection = correctionIacBatVoltage();
 if (currentStatus.IacbatCorrection != 100) { sumCorrections = div100(sumCorrections * currentStatus.IacbatCorrection); }

   currentStatus.BIEGActiveCorrection = correctionBIEGActivePWvoltage();
 if (currentStatus.BIEGActiveCorrection != 100) { sumCorrections = div100(sumCorrections * currentStatus.BIEGActiveCorrection); }
...
static inline byte correctionsFuel_new()
{
  ...
  currentStatus.IacbatCorrection = correctionIacBatVoltage(); numCorrections++;
  currentStatus.BIEGActiveCorrection = correctionBIEGActivePWvoltage(); numCorrections++;
...
/** Battery IAC Voltage correction.
 *  
 */
byte correctionIacBatVoltage()
{
  byte idlebat = 100;
  idlebat = table2D_getValue(&idleVCorrectionTable,  currentStatus.battery10);
  return idlebat;
}

byte correctionBIEGActivePWvoltage()
{
  byte BIEGPW = 100;
  BIEGPW = get3DTableValue(&wmiTable, currentStatus.MAP, currentStatus.RPM);
  return BIEGPW;
}
