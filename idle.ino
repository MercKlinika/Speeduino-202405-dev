...
void idleControl()
{
  ...
  //Check whether the BIEG position sensor is active
  if(configPage15.BIEGEnabled == true)
  {
    if(configPage15.BIEGPolarity == 0) { currentStatus.BIEGActive = !digitalRead(pinBIEG); } //Normal mode (ground switched)
    else { currentStatus.BIEGActive = digitalRead(pinBIEG); } //Inverted mode (5v activates closed throttle position sensor)
  }
  else { currentStatus.BIEGActive = 0; }
  ...
  switch(configPage6.iacAlgorithm)
  {
    ...
    case IAC_ALGORITHM_PWM_OL:      //Case 2 is PWM open loop
      ...
      if( BIT_CHECK(currentStatus.engine, BIT_ENGINE_CRANK) )
      {
        ...
        currentStatus.idleLoad = table2D_getValue(&iacCrankDutyTable, currentStatus.coolant + CALIBRATION_TEMPERATURE_OFFSET) + currentStatus.IacbatCorrection; //All temps are offset by 40 degrees
        idleTaper = 0;
      }
      ...
      else
      {
        ...
        {
          //Standard running
          currentStatus.idleLoad = table2D_getValue(&iacPWMTable, currentStatus.coolant + CALIBRATION_TEMPERATURE_OFFSET) + currentStatus.IacbatCorrection; //All temps are offset by 40 degrees
          
         if( (currentStatus.RPM > (unsigned int)(configPage15.zaworRPM * 100)) && (currentStatus.TPS >= configPage15.zaworTPSMax) || (configPage6.iacPWMrun == false) ){ disableIdle();} 
        }
      ...
      if(currentStatus.BIEGActive == true) { currentStatus.idleLoad += currentStatus.BIEGActiveCorrection; } //Add Idle Up amount if active
      if( currentStatus.idleLoad > 100 ) { currentStatus.idleLoad = 100; } //Safety Check
      idle_pwm_target_value = percentage(currentStatus.idleLoad, idle_pwm_max_count);
